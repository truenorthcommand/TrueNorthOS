...
export async function seedDatabase(): Promise<{ message: string; counts: Record<string, number> }> {
  ...
  try {
    ...
    // ============================================================
    const seedQuotes = [
      { clientId: insertedClients[0].id, quoteNumber: "QT-2026-001", title: "Kitchen Refurbishment - 15 Oak Street", description: "Full kitchen renovation including units, worktops, tiling, electrical, and plumbing.", status: "accepted", total: 12500, createdAt: new Date('2026-03-01') },
      { clientId: insertedClients[0].id, quoteNumber: "QT-2026-002", title: "Bathroom Upgrade - 7 Birch Lane", description: "New bathroom suite, tiling, heated towel rail, and extractor fan.", status: "pending", total: 4800, createdAt: new Date('2026-04-20') },
      { clientId: insertedClients[1].id, quoteNumber: "QT-2026-003", title: "Communal Heating Overhaul - Riverside House", description: "Replace aging communal heating system. New boiler plant, pipework, and controls for 36 units.", status: "sent", total: 85000, createdAt: new Date('2026-04-15') },
      { clientId: insertedClients[1].id, quoteNumber: "QT-2026-004", title: "Major Works Programme - Crescent Court", description: "Phase 1: Roof replacement, external redecoration, window overhaul. 42-unit block.", status: "pending", total: 450000, createdAt: new Date('2026-04-25') },
      { clientId: insertedClients[2].id, quoteNumber: "QT-2026-005", title: "Smart Home Extension - Penthouse 1", description: "Add Crestron zone for new home office. Automated blinds, lighting scenes, climate control.", status: "accepted", total: 8500, createdAt: new Date('2026-04-22') },
      { clientId: insertedClients[3].id, quoteNumber: "QT-2026-006", title: "HMO Fire Safety Upgrade - 42 Fallowfield Road", description: "Fire door replacements (5), emergency lighting, fire alarm panel upgrade.", status: "sent", total: 6200, createdAt: new Date('2026-04-28') },
      { clientId: insertedClients[4].id, quoteNumber: "QT-2026-007", title: "AC System Replacement - MediaCity Suite", description: "Replace 4x aging split systems with VRF system. Improved efficiency and zone control.", status: "pending", total: 22000, createdAt: new Date('2026-05-01') },
    ];
    ...
    // ============================================================
    const seedInvoices = [
      { clientId: insertedClients[0].id, invoiceNumber: "INV-2026-001", status: "paid", total: 6250, dueDate: new Date('2026-04-01'), createdAt: new Date('2026-03-15'), description: "Kitchen Refurbishment Deposit (50%) - 15 Oak Street" },
      { clientId: insertedClients[0].id, invoiceNumber: "INV-2026-002", status: "sent", total: 6250, dueDate: new Date('2026-05-15'), createdAt: new Date('2026-05-01'), description: "Kitchen Refurbishment Final Payment - 15 Oak Street" },
      { clientId: insertedClients[0].id, invoiceNumber: "INV-2026-003", status: "paid", total: 180, dueDate: new Date('2026-04-15'), createdAt: new Date('2026-03-15'), description: "Annual Gas Safety Inspection - 15 Oak Street" },
      { clientId: insertedClients[0].id, invoiceNumber: "INV-2026-004", status: "sent", total: 1850, dueDate: new Date('2026-05-30'), createdAt: new Date('2026-05-01'), description: "DPC Injection & Replastering - 7 Birch Lane" },
      { clientId: insertedClients[1].id, invoiceNumber: "INV-2026-005", status: "paid", total: 4320, dueDate: new Date('2026-04-30'), createdAt: new Date('2026-04-10'), description: "Gas Safety Inspections - Block A (18 units completed)" },
      { clientId: insertedClients[1].id, invoiceNumber: "INV-2026-006", status: "overdue", total: 2800, dueDate: new Date('2026-04-20'), createdAt: new Date('2026-04-10'), description: "Emergency Heating Repair - Riverside House" },
      { clientId: insertedClients[2].id, invoiceNumber: "INV-2026-007", status: "paid", total: 450, dueDate: new Date('2026-05-01'), createdAt: new Date('2026-04-20'), description: "Smart Home Maintenance - Penthouse 1, Deansgate Square" },
      { clientId: insertedClients[3].id, invoiceNumber: "INV-2026-008", status: "paid", total: 380, dueDate: new Date('2026-04-20'), createdAt: new Date('2026-04-05'), description: "Emergency Callout - Burst Pipe - 42 Fallowfield Road" },
      { clientId: insertedClients[3].id, invoiceNumber: "INV-2026-009", status: "sent", total: 950, dueDate: new Date('2026-05-30'), createdAt: new Date('2026-05-01'), description: "Damp Treatment & Repointing - 3 Moss Lane East" },
      { clientId: insertedClients[4].id, invoiceNumber: "INV-2026-010", status: "sent", total: 3200, dueDate: new Date('2026-05-28'), createdAt: new Date('2026-04-28'), description: "Heritage Rainwater Goods Replacement - 15 King Street" },
    ];
